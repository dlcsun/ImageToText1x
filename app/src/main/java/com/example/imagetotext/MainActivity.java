        ...
        if(requestCode==CropImage.CROP_IMAGE_ACTIVITY_REQUEST_CODE){
            ...
            if(resultCode==RESULT_OK){
                ...
                if(!recognizer.isOperational()){
                    ...
                    int totals = 0;
                    result test = new result(null, null, 0, 0, -1, 0, 0, 0);
                    //ArrayList<result> results = new ArrayList<result>();
                    //String out = "";
                    while (!endoffile){
                        test = getlines(items, ".*\\btotal\\b.*|.*\\bbalance\\b.*", ".*\\$?\\s*[0-9]+(\\.[0-9][0-9])?$", test.keyblock, test.keyline, test.keyword + 1);//set text to edit text
            ...
            if (section != null && section.getValue() != null) {
                ...
                for (; j < lines.size() && key == null; j++) {    //these are lines
                    ...
                    if(line != null && line.getValue() != null) {
                        ...
                        if(i == blockplace && j == lineplace){
                            k = wordplace;
                        }
                        else{
                            k = 0;
                        }
                        for (; k < words.size() && key == null; k++) {    //these are words
                            Log.d("Processing", "    k = " + k + ": " + words.get(k).getValue() + " [" + words.get(k).getBoundingBox().left + ", " + words.get(k).getBoundingBox().top + "]");
                            //boolean sameLine =Math.abs(word.getBoundingBox().top-total.getBoundingBox().top)<10;
                            Text word = words.get(k);
                            if (word.getValue().toLowerCase().matches(rawregexname)) {
                                key = word;
            ...
            for (i2 = 0; i2 < items.size() && !complete; i2++) {     //these are blocks
                ...
                if (section != null && section.getValue() != null) {
                    ...
                    for (j2 = 0; j2 < lines.size() && !complete; j2++) {    //these are lines
                        ...
                        if (line != null && line.getValue() != null) {
                            List<? extends Text> words = line.getComponents();
                            for (k2 = 0; k2 < words.size() && !complete; k2++) {    //these are words
                                Text word = words.get(k2);
                                boolean sameLine = Math.abs(word.getBoundingBox().top - key.getBoundingBox().top) < 100 && word.getBoundingBox().left > key.getBoundingBox().right;
                                if (word.getValue().toLowerCase().matches(rawregexprice) && sameLine) {
                                    if(price == null || word.getBoundingBox().left < price.getBoundingBox().left) {
                                        price = word;
                                        out.priceblock = i2 - 1;
                                        out.priceline = j2 - 1;
                                        out.priceword = k2 - 1;
